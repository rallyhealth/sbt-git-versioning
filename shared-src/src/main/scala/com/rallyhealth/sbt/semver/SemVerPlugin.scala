...
import com.rallyhealth.sbt.versioning.{GitVersioningPlugin, ReleaseVersion, SemVerReleaseType, SemanticVersion}
...
object SemVerPlugin extends AutoPlugin {
  ...
  override lazy val projectSettings: Seq[Setting[_]] = Seq(
    ...
    semVerCheck := {
      ...
      if (semVerVersionLimit.value != "deprecated") {
        log.warn(
          """'semVerVersionLimit' is DEPRECATED and will be ignored.
            |
     ...
     * FAAAR better than not running at all. If you do change this YOU MUST RUN rally-versioning-test/checkSemver.sh
    ...
    MiMa.miMaChecker := {
      ...
      new MimaChecker(MiMaExecutorShim(classpath, streams.value))
    },

    MiMa.moduleResolver := new IvyModuleResolver(scalaModuleInfo.value, ivySbt.value, streams.value),

    MiMa.maybePrevRelease := GitVersioningPlugin.autoImport.gitVersioningPreviousRelease.value,
