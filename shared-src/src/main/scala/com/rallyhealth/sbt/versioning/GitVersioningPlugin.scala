...
import sbtdynver.{DynVer, DynVerPlugin}
...
object GitVersioningPlugin extends AutoPlugin {
  ...
  /**
    ...
    *
    * Similarly require the [[DynVerPlugin]] so we set the version not it
    */
...
//      "generate-version",
//      "Writes the version to a rally-version.properties file.")
    ...
    lazy val gitVersioningPreviousRelease = settingKey[Option[ReleaseVersion]]("The last stable tag")
  }
  ...
  override lazy val buildSettings: Seq[Setting[_]] = Seq(
    ...
    versionFromGit := {
      ...
      val dynVer = DynVer(Some((baseDirectory in ThisBuild).value), dynverSeparator.value, dynverVTagPrefix.value).makeDynVer(new Date)

      // The orElse represents a newly initialized repo, next version to different from the version it is to go to the version you will be
      val gitVersion = dynVer.flatMap(v => SemanticVersion.parseDynVerString(v))
        .getOrElse(ReleaseVersion.initialVersion) match {
          case ver: SnapshotVersion => ver.copy(patch = ver.patch + 1)
          case other => other
        }

      sLog.value.info(s"GitVersioningPlugin set versionFromGit=$gitVersion")
    ...
    versionOverride := {
      Option(System.getProperty("version.override")).map {
        versionOverrideStr =>
          ...
          sLog.value.info(s"GitVersioningPlugin set versionOverride=$versionOverrideStr")
          ver.toString
      }
    },
    ...
    semanticVersion := {
      ...
      val described = DynVer(Some((baseDirectory in ThisBuild).value), dynverSeparator.value, dynverVTagPrefix.value).getGitDescribeOutput(new Date)
      val hash = described.map(_.commitSuffix.sha).getOrElse("0" * 8)
      val hashAndCount = HashAndCount(if (hash.isEmpty) "0" * 8 else hash, CommitsSemVerIdentifier(described.map(_.commitSuffix.distance).getOrElse(0)))
      ...
      lazy val boundedVersionFromGit: SemanticVersion = {
        ...
        val applySnapshotLowerBound: VersionTransform = ver => gitVersioningSnapshotLowerBound.value
          .foldLeft(ver)(_.lowerBound(_, hashAndCount))
    ...
    gitVersioningPreviousRelease := {
      val prevVersion = DynVer(Some((baseDirectory in ThisBuild).value), dynverSeparator.value, dynverVTagPrefix.value).previousVersion
        .flatMap(v => SemanticVersion.parseDynVerString(v)).map(_.toRelease)
...
//  /**
//    * Generates a rally-version.properties file in the managedResources directory suitable for serving version requests during compile.
//    */
...
//
//      val file = (resourceManaged in Compile).value / "rally-version.properties"
