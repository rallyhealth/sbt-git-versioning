...
//https://github.com/lightbend/mima/commit/2f4d37463bbc976ed97413a974f48bb9997ae404
...
/**
  ...
  * - Less stringly-typed
  */
class CustomMiMaLibMiMaExecutor(classpath: ClassPath) extends MiMaExecutor {
  ...
  private def classPath(name: String): Option[ClassPath] = {
    val f = new File(new java.io.File(name))
    Option(AbstractFile.getDirectory(f)).map(dirClassPath)
  }
